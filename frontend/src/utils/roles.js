...
const ROLE_ACCESS_ENCODING_GATEPASS = ['/gatepass', '/gatepass/history'];
...
const ROLE_ACCESS = {
  /** Gate: scan page only. Transmittal: release Scan Barcode only (not receptionist/recipient). */
  scan_only: ['/gatepass/scan', '/transmittal/scan'],
  /** Transmittal only: Recipient Scan. */
...
export function getDefaultPath(role, section = SECTION_GATEPASS, user = null) {
  ...
  if (role === 'scan_only') {
    if (section === SECTION_TRANSMITTAL) {
      return isReceptionDeskUser(user) ? '/transmittal/receptionist' : '/transmittal/scan';
    }
...
/** Nav items for role and section. Admin sees User/Product for Gate Pass; encoding sees Form + History only. */
